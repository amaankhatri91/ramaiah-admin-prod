...
const OurJourneySection = () => {
  ...
  const [initialFormValues, setInitialFormValues] =
    useState<OurJourneyFormSchema | null>(null);
  ...
  const getInitialValues = (): OurJourneyFormSchema => {
    ...
    const journeyContentBlocks = contentBlocks.filter(
      (block: ContentBlock) => block.section_id === 7
    );
    const sortedBlocks = journeyContentBlocks.sort(
      (a, b) => a.display_order - b.display_order
    );
    const titleBlock = sortedBlocks.find(
      (block: ContentBlock) =>
        block.display_order === 1 && block.block_type === "text"
    );
    const imageBlock = sortedBlocks.find(
      (block: ContentBlock) =>
        block.display_order === 3 && block.block_type === "image"
    );
    ...
    try {
      ...
      if (result.status === 1) {
        ...
        if (responseData?.savedMedia?.id) {
          ...
          console.log(
            "Our Journey file upload - savedMedia.id:",
        ...
        setFieldValue(
          "altText",
          responseData?.savedMedia?.alt_text || currentAltText || ""
        );
  ...
  const onSubmit = async (values: OurJourneyFormSchema) => {
    try {
      ...
      const contentBlock = sortedBlocks.find(
        (block: ContentBlock) => block.display_order === 2 && block.content
      );
      const imageBlock = sortedBlocks.find(
        (block: ContentBlock) =>
          block.display_order === 3 && block.block_type === "image"
      ...
      const headerTextChanged =
        titleBlock && values.headerText !== initialValues.headerText;
      ...
      const contentChanged =
        contentBlock && values.content !== initialValues.content;
      ...
      const uploadFileMediaIdChanged =
        values.uploadFileMediaId !== initialValues.uploadFileMediaId;
      const altTextChanged =
        (values.altText || "") !== (initialValues.altText || "");
      const uploadFileOverallChanged =
        imageBlock &&
        (uploadFileChanged || uploadFileMediaIdChanged || altTextChanged);

      console.log("Upload File comparison:", {
        file: {
          current: values.uploadFile,
          initial: initialValues.uploadFile,
          changed: uploadFileChanged,
        },
        mediaFileId: {
          current: values.uploadFileMediaId,
          initial: initialValues.uploadFileMediaId,
          changed: uploadFileMediaIdChanged,
        },
        altText: {
          current: values.altText,
          initial: initialValues.altText,
      ...
      if (uploadFileOverallChanged) {
        ...
        const mediaFileIdToUse =
          values.uploadFileMediaId ||
          imageBlock.media_files?.[0]?.media_file?.id;
      ...
      if (result.success) {
        ...
        throw new Error(
          result.message || "Failed to update our journey section"
        );
      }
    } catch (error: any) {
      const errorMessage =
        error?.data?.message ||
        error?.message ||
        "Failed to update our journey section";
      toast.push(
        <Notification type="danger" duration={3000} title="Error">
          {errorMessage}
        </Notification>,
        ...
        {isLoadingData ? (
          <div className="flex justify-center items-center py-8">
            <div className="text-gray-500">
              Loading our journey section data...
            </div>
          </div>
        ) : error ? (
          <div className="flex justify-center items-center py-8">
            <div className="text-red-500">
              Error loading our journey section data
            </div>
          </div>
        ) : (
          <Formik
                ...
                <FormContainer>
                  ...
                  <div className="mb-6">
                    <FormItem
                      ...
                      labelClass="text-[#495057] font-inter text-[14px] font-medium leading-normal"
                      invalid={
                        (errors.uploadFile && touched.uploadFile) as boolean
                      }
