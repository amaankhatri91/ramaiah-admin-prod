  ...
  certificates: Yup.array()
    .of(
      Yup.object().shape({
        ...
        nameHeadingLevel: Yup.string().required(
          "Certificate name heading level is required"
        ),
...
const AccreditationsSection = () => {
  ...
  const [isCertificateUploading, setIsCertificateUploading] = useState<{
    [key: string]: boolean;
  }>({});
  const fileInputRefs = useRef<{ [key: string]: HTMLInputElement | null }>({});
  const [initialFormValues, setInitialFormValues] =
    useState<AccreditationsFormSchema | null>(null);
  ...
  const getInitialValues = (): AccreditationsFormSchema => {
    ...
    const accreditationBlocks = contentBlocks.filter(
      (block: ContentBlock) => block.section_id === 5
    );

    // Get header text from text block type
    const textBlock = accreditationBlocks.find(
    ...
    const imageBlocks = accreditationBlocks.filter(
      (block: ContentBlock) => block.block_type === "image"
    );
    const certificates =
  ...
  const handleCertificateChange = (
    ...
    certificates: Certificate[],
    id: string,
    field: "name" | "image" | "nameHeadingLevel" | "altText",
    value: string
  ) => {
        ...
        const updatedCertificates = certificates.map((cert) => {
          if (cert.id === id) {
            ...
            if (responseData?.savedMedia?.id) {
              ...
              console.log(
                "Certificate file upload - savedMedia.id:",
            ...
            updatedCert.altText =
              responseData?.savedMedia?.alt_text || cert.altText || "";
  ...
  const onSubmit = async (values: AccreditationsFormSchema) => {
    try {
      ...
      if (headerTextChanged || headerTextHeadingLevelChanged) {
        ...
        const existingTextBlock = accreditationBlocks.find(
          (block: ContentBlock) => block.block_type === "text"
        );
      ...
      if (certificatesChanged) {
        ...
        values.certificates.forEach((certificate, index) => {
          ...
          const nameChanged =
            !initialCertificate || initialCertificate.name !== certificate.name;
          const nameHeadingLevelChanged =
            !initialCertificate ||
            initialCertificate.nameHeadingLevel !==
              certificate.nameHeadingLevel;
          const mediaFileIdChanged =
            !initialCertificate ||
            initialCertificate.media_file_id !== certificate.media_file_id;
          const altTextChanged =
            !initialCertificate ||
            initialCertificate.altText !== certificate.altText;
          const hasChanges =
            nameChanged ||
            nameHeadingLevelChanged ||
            mediaFileIdChanged ||
            altTextChanged;

          if (hasChanges) {
            // Find existing image block for this certificate by ID (titles may repeat like "JCI")
            const existingBlock = accreditationBlocks.find(
              (block: ContentBlock) =>
            ...
            if (nameHeadingLevelChanged) {
              ...
              if (customCss.match(/heading-level:\s*h[1-6]/i)) {
                customCss = customCss.replace(
                  /heading-level:\s*h[1-6]/i,
                  `heading-level:${certificate.nameHeadingLevel}`
                );
              } else {
                customCss = customCss
                  ? `${customCss}; heading-level:${certificate.nameHeadingLevel}`
                  : `heading-level:${certificate.nameHeadingLevel}`;
              }
              ...
              contentBlock.media_files = [
                {
                  ...
                  media_file_id:
                    certificate.media_file_id ||
                    existingMediaFile?.media_file?.id,
      ...
      if (result.success) {
        ...
        throw new Error(
          result.message || "Failed to update accreditations section"
        );
      }
    } catch (error: any) {
      const errorMessage =
        error?.data?.message ||
        error?.message ||
        "Failed to update accreditations section";
      toast.push(
        <Notification type="danger" duration={3000} title="Error">
          {errorMessage}
        </Notification>,
        ...
        {isLoadingData ? (
          <div className="flex justify-center items-center py-8">
            <div className="text-gray-500">
              Loading accreditations section data...
            </div>
          </div>
        ) : error ? (
          <div className="flex justify-center items-center py-8">
            <div className="text-red-500">
              Error loading accreditations section data
            </div>
          </div>
        ) : (
          <Formik
              ...
              <Form>
                <FormContainer>
                  ...
                  <div className="flex items-center justify-between mb-6">
                    <p className="text-[#495057] font-inter text-[14px] font-semibold leading-normal">
                      Our Accreditations & Certifications
                    </p>
                    <Button
                      type="button"
                      onClick={() =>
                        handleAddCertificate(setFieldValue, values.certificates)
                      }
                          ...
                          <FormItem
                            ...
                            invalid={
                              (errors.certificates?.[index] &&
                                typeof errors.certificates[index] ===
                                  "object" &&
                                (errors.certificates[index] as any)?.name &&
                                touched.certificates?.[index]?.name) as boolean
                            }
                            errorMessage={
                              errors.certificates?.[index] &&
                              typeof errors.certificates[index] === "object"
                                ? String(
                                    (errors.certificates[index] as any)?.name ||
                                      ""
                                  )
                            ...
                            <div className="flex flex-col w-full sm:flex-row sm:items-center sm:justify-between rounded-[24px] border-[0.75px] border-[#CED4DA] mr-[20px]">
                              ...
                              <div className="flex flex-col sm:flex-row gap-3">
                                <label className="cursor-pointer">
                                  <input
                                    ref={(el) =>
                                      (fileInputRefs.current[certificate.id] =
                                        el)
                                    }
                                    type="file"
                                  ...
                                  <Button
                                    type="button"
                                    loading={
                                      isCertificateUploading[certificate.id]
                                    }
                                    onClick={() =>
