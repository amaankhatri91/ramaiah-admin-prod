...
const QuickLinksSection = () => {
  ...
  const [initialFormValues, setInitialFormValues] =
    useState<QuickLinksFormSchema | null>(null);
  ...
  const getInitialValues = (): QuickLinksFormSchema => {
    ...
    const getHeadingLevel = (
      ...
      // Find all heading-level occurrences (h1-h6, p, span), case-insensitive
      const matches = block.custom_css.match(
        /heading-level\s*:\s*(h[1-6]|p|span)\b/gi
      );
      if (!matches || matches.length === 0) return defaultValue;

      // Use the last occurrence (in case there are duplicates)
      const last = matches[matches.length - 1];
      const tagMatch = last.match(/(h[1-6]|p|span)/i);
    ...
    if (!contentBlocks || contentBlocks.length === 0) {
      return {
        quickLinks: [
          {
            id: "1",
            name: "Book OPD Appointments (9 am - 5 pm)",
            link: "",
            icon: "icon_01.svg",
          ...
          {
            id: "2",
            name: "Book Prime Clinic Appointments (5 pm - 8 pm)",
            link: "",
            icon: "icon_01.svg",
            mediaFileId: undefined,
            headingLevel: "h3",
    ...
    const quickLinkBlocks =
      contentBlocks.filter((block: ContentBlock) => block.section_id === 2) ||
      [];
    ...
    return {
      quickLinks:
        quickLinksWithHeadingLevels.length > 0
          ? quickLinksWithHeadingLevels
          : [
              {
                id: "1",
                name: "Book OPD Appointments (9 am - 5 pm)",
  ...
  const handleQuickLinkChange = (
    ...
    quickLinks: QuickLink[],
    id: string,
    field: "name" | "link" | "icon" | "headingLevel" | "altText",
    value: string
  ) => {
    ...
    try {
      ...
      if (result.status === 1) {
        ...
        const finalUpdatedLinks = quickLinks.map((link) =>
          link.id === id
            ? {
                ...
                altText:
                  responseData?.savedMedia?.alt_text || link.altText || "",
              }
            : link
        );
        ...
        if (responseData?.savedMedia?.id) {
          console.log(
            "Quick link icon upload - savedMedia.id:",
    ...
    try {
      ...
      const quickLinkBlocks =
        allContentBlocks.filter(
          (block: ContentBlock) => block.section_id === 2
        ) || [];
      ...
      values.quickLinks.forEach((link, index) => {
        ...
        const headingLevelChanged =
          link.headingLevel !== initialLink?.headingLevel;
        ...
        if (
          ...
          console.log(`Quick Link ${index + 1} changed:`, {
            name: {
              current: link.name,
              initial: initialLink?.name,
              changed: nameChanged,
            },
            link: {
              current: link.link,
              initial: initialLink?.link,
              changed: linkChanged,
            },
            icon: {
              current: link.mediaFileId,
              initial: initialLink?.mediaFileId,
              changed: iconChanged,
            },
            headingLevel: {
              current: link.headingLevel,
              initial: initialLink?.headingLevel,
              changed: headingLevelChanged,
            },
            altText: {
              current: link.altText,
              initial: initialLink?.altText,
          ...
          if ((headingLevelChanged || nameChanged) && link.headingLevel) {
            ...
            if (customCss.match(/heading-level:\s*h[1-6]/i)) {
              customCss = customCss.replace(
                ...
                `heading-level:${link.headingLevel}`
              );
            } else {
              customCss = customCss
                ? `${customCss}; heading-level:${link.headingLevel}`
                : `heading-level:${link.headingLevel}`;
            }
            ...
            contentBlock.media_files = [
              {
                ...
                media_file_id:
                  link.mediaFileId || existingMediaFile?.media_file?.id,
                ...
                <FormContainer>
                  <div className="">
                    {values.quickLinks.map((link, index) => (
                      <div
                        key={link.id}
                        className="grid grid-cols-1 lg:grid-cols-3 gap-4 mt-0"
                      >
                        {/* Name of Box */}
                        <div>
                          <FormItem
                            label={`Name of ${index + 1}${getOrdinalSuffix(
                              index
                            )} Box`}
                            labelClass="text-[#495057] font-inter text-[14px] font-medium leading-normal"
                            invalid={
                              (errors.quickLinks?.[index] &&
                                typeof errors.quickLinks[index] === "object" &&
                                (errors.quickLinks[index] as any)?.name &&
                                touched.quickLinks?.[index]?.name) as boolean
                            }
                            errorMessage={
                              errors.quickLinks?.[index] &&
                              typeof errors.quickLinks[index] === "object"
                            ...
                            <div className="flex gap-3">
                              <Select
                                ...
                                value={headingLevelOptions.find(
                                  (option) =>
                                    option.value === (link.headingLevel || "h3")
                                )}
                              />
                              <Input
                                value={link.name}
                        ...
                        <div>
                          <FormItem
                            ...
                            invalid={
                              (errors.quickLinks?.[index] &&
                                typeof errors.quickLinks[index] === "object" &&
                                (errors.quickLinks[index] as any)?.link &&
                                touched.quickLinks?.[index]?.link) as boolean
                            }
                            errorMessage={
                              errors.quickLinks?.[index] &&
                              typeof errors.quickLinks[index] === "object"
                            ...
                            <div className="flex flex-col w-full sm:flex-row sm:items-center sm:justify-between rounded-[24px] border-[0.75px] border-[#CED4DA] mr-[20px]">
                              ...
                              <div className="flex flex-col sm:flex-row gap-3">
                                ...
                                <Button
                                  ...
                                  onClick={() =>
                                    document
                                      .getElementById(`file-upload-${link.id}`)
                                      ?.click()
                                  }
