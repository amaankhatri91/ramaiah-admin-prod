...
const validationSchema = Yup.object().shape({
  ...
  storyBoxes: Yup.array()
    .of(
      Yup.object().shape({
        header: Yup.string().required("Header is required"),
        headerHeadingLevel: Yup.string().required(
          "Header heading level is required"
        ),
...
const OurStorySection = () => {
  ...
  const [initialFormValues, setInitialFormValues] =
    useState<OurStoryFormSchema | null>(null);
  ...
  const getHeaderHeadingLevel = (
    block: ContentBlock | undefined,
    defaultValue: string
  ): string => {
    if (!block?.custom_css) return defaultValue;

    // Match all valid header-heading-level definitions (h1–h6, p, span)
    const matches = block.custom_css.match(
      /header-heading-level\s*:\s*(h[1-6]|p|span)\b/gi
    );
  ...
  const getSubHeaderHeadingLevel = (
    block: ContentBlock | undefined,
    defaultValue: string
  ): string => {
    if (!block?.custom_css) return defaultValue;
    const match = block.custom_css.match(
      /subheader-heading-level:\s*(h[1-6])/i
    );
    return match ? match[1].toLowerCase() : defaultValue;
  };
  ...
  const getInitialValues = (): OurStoryFormSchema => {
    ...
    const storyBlocks = contentBlocks.filter(
      (block: ContentBlock) => block.section_id === 4
    );

    // Get header text from text block type
    const textBlock = storyBlocks.find(
    ...
    const statisticBlocks = storyBlocks.filter(
      (block: ContentBlock) => block.block_type === "statistic"
    );
    ...
    try {
      ...
      if (result.status === 1) {
        ...
        const finalUpdatedBoxes = storyBoxes.map((box) =>
          box.id === id
            ? {
                ...
                altText:
                  responseData?.savedMedia?.alt_text || box.altText || "",
              }
            : box
        );
        ...
        if (responseData?.savedMedia?.id) {
          console.log(
            "Story box icon upload - savedMedia.id:",
  ...
  const onSubmit = async (values: OurStoryFormSchema) => {
    try {
      ...
      const headerBlock = storyBlocks.find(
        (block: ContentBlock) => block.block_type === "text"
      );
      const statisticBlocks = storyBlocks.filter(
        (block: ContentBlock) => block.block_type === "statistic"
      ...
      const headerTextChanged =
        headerBlock && values.headerText !== initialValues.headerText;
      ...
      values.storyBoxes.forEach((box, index) => {
        ...
        if (
          ...
          console.log(`Story Box ${index + 1} changed:`, {
            header: {
              current: box.header,
              initial: initialBox.header,
              changed: headerChanged,
            },
            headerHeadingLevel: {
              current: box.headerHeadingLevel,
              initial: initialBox.headerHeadingLevel,
              changed: headerHeadingLevelChanged,
            },
            subHeader: {
              current: box.subHeader,
              initial: initialBox.subHeader,
              changed: subHeaderChanged,
            },
            subHeaderHeadingLevel: {
              current: box.subHeaderHeadingLevel,
              initial: initialBox.subHeaderHeadingLevel,
              changed: subHeaderHeadingLevelChanged,
            },
            icon: {
              current: box.mediaFileId,
              initial: initialBox.mediaFileId,
              changed: iconChanged,
            },
            altText: {
              current: box.altText,
              initial: initialBox?.altText,
              changed: altTextChanged,
            },
          });
          ...
          if (headerHeadingLevelChanged) {
            if (customCss.match(/header-heading-level:\s*h[1-6]/i)) {
              customCss = customCss.replace(
                /header-heading-level:\s*h[1-6]/i,
                `header-heading-level:${box.headerHeadingLevel}`
              );
            } else {
              customCss = customCss
                ? `${customCss}; header-heading-level:${box.headerHeadingLevel}`
                : `header-heading-level:${box.headerHeadingLevel}`;
            }
          ...
          if (subHeaderHeadingLevelChanged) {
            if (customCss.match(/subheader-heading-level:\s*h[1-6]/i)) {
              customCss = customCss.replace(
                /subheader-heading-level:\s*h[1-6]/i,
                `subheader-heading-level:${box.subHeaderHeadingLevel}`
              );
            } else {
              customCss = customCss
                ? `${customCss}; subheader-heading-level:${box.subHeaderHeadingLevel}`
                : `subheader-heading-level:${box.subHeaderHeadingLevel}`;
            }
            ...
            contentBlock.media_files = [
              {
                ...
                content_block_id: correspondingBlock.id,
                media_file_id:
                  box.mediaFileId || existingMediaFile?.media_file?.id,
      ...
      const errorMessage =
        error?.data?.message ||
        error?.message ||
        "Failed to update our story section";
      toast.push(
        <Notification type="danger" duration={3000} title="Error">
          {errorMessage}
        </Notification>,
        ...
        {isLoadingData ? (
          <div className="flex justify-center items-center py-8">
            <div className="text-gray-500">
              Loading our story section data...
            </div>
          </div>
        ) : error ? (
          <div className="flex justify-center items-center py-8">
            <div className="text-red-500">
              Error loading our story section data
            </div>
          </div>
        ) : (
          <Formik
                  ...
                  <div className="">
                    {values.storyBoxes.map((box, index) => (
                      ...
                      >
                        ...
                        <div>
                          <FormItem
                            label={`Header of ${index + 1}${getOrdinalSuffix(
                              index
                            )} Box`}
                            labelClass="text-[#495057] font-inter text-[14px] font-medium leading-normal"
                            invalid={
                              (errors.storyBoxes?.[index] &&
                                typeof errors.storyBoxes[index] === "object" &&
                                (errors.storyBoxes[index] as any)?.header &&
                                touched.storyBoxes?.[index]?.header) as boolean
                            }
                            errorMessage={
                              errors.storyBoxes?.[index] &&
                              typeof errors.storyBoxes[index] === "object"
                            ...
                            <div className="flex gap-3">
                              <Select
                                ...
                                value={headingLevelOptions.find(
                                  (option) =>
                                    option.value === box.headerHeadingLevel
                                )}
                              />
                              <Input
                                value={box.header}
                        ...
                        <div>
                          <FormItem
                            label={`Sub header of ${
                              index + 1
                            }${getOrdinalSuffix(index)} Box`}
                            labelClass="text-[#495057] font-inter text-[14px] font-medium leading-normal"
                            invalid={
                              (errors.storyBoxes?.[index] &&
                                typeof errors.storyBoxes[index] === "object" &&
                                (errors.storyBoxes[index] as any)?.subHeader &&
                                touched.storyBoxes?.[index]
                                  ?.subHeader) as boolean
                            }
                            errorMessage={
                              errors.storyBoxes?.[index] &&
                              typeof errors.storyBoxes[index] === "object"
                                ? String(
                                    (errors.storyBoxes[index] as any)
                                      ?.subHeader || ""
                                  )
                                : undefined
                            }
                          >
                            <div className="flex gap-3">
                              <Select
                                ...
                                value={headingLevelOptions.find(
                                  (option) =>
                                    option.value === box.subHeaderHeadingLevel
                                )}
                              />
                              <Input
                                value={box.subHeader}
                        ...
                        <div>
                          <label className="block text-[#495057] font-inter text-[14px] font-medium leading-normal mb-2">
                            {index + 1}
                            {getOrdinalSuffix(index)} Box Icon
                          </label>
                          <div className="flex w-full">
                            <div className="flex flex-col w-full sm:flex-row sm:items-center sm:justify-between rounded-[24px] border-[0.75px] border-[#CED4DA] mr-[20px]">
                              ...
                              <div className="flex flex-col sm:flex-row gap-3">
                                <input
                                  ref={(el) =>
                                    (fileInputRefs.current[box.id] = el)
                                  }
                                  type="file"
                                  accept="image/*,.svg"
                                  onChange={(e) =>
                                ...
                                <Button
                                  type="button"
                                  loading={
                                    uploadingBoxId === box.id && isUploading
                                  }
