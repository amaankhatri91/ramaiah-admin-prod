...
const validationSchema = Yup.object().shape({
  ...
  subHeaderHeadingLevel: Yup.string().required(
    "Sub header heading level is required"
  ),
...
const HeroSection = () => {
  ...
  const [bannerToDelete, setBannerToDelete] = useState<BannerImage | null>(
    null
  );
  ...
  const [initialFormValues, setInitialFormValues] =
    useState<HeroSectionFormSchema | null>(null);
  ...
  const getInitialValues = (): HeroSectionFormSchema => {
    ...
    console.log(contentBlocks, "Check Block");
    ...
    const heroBlocks = contentBlocks.filter(
      (block: ContentBlock) => block.section_id === 1
    );
    const headlineBlock = heroBlocks.find(
      (block: ContentBlock) => block.title === "Hero Headline"
    );
    const subtitleBlock = heroBlocks.find(
      (block: ContentBlock) => block.title === "Hero Subtitle"
    );
    ...
    const getHeadingLevel = (
      ...
      // Match all heading-level definitions
      ...
      // Take the LAST occurrence (in case of duplicates)
      const lastMatch = matches[matches.length - 1];

      // Extract tag name (h1–h6, p, span)
    ...
    const bannerBlock = contentBlocks.find(
      (block: ContentBlock) =>
        block.section_id === 1 &&
        block.media_files &&
    ...
    const bannerImages = bannerBlock?.media_files?.map(
    ...
    return {
      ...
      smallBannerFile:
        heroData.smallBannerFile || "Ramaiah_introduction Video.mp4",
  ...
  const handleBannerModalSubmit = (
    values: BannerModalFormSchema,
    mainSetFieldValue: any,
    mainBannerImages: BannerImage[]
  ) => {
    if (editingBanner) {
    ...
    try {
      ...
      if (result.status === 1) {
        ...
        if (responseData?.savedMedia?.id) {
          ...
          console.log(
            "Banner file upload - savedMedia.id:",
  ...
  const handleDeleteBanner = (
    ...
    setFieldValue(
      "bannerImages",
      bannerImages.filter((banner) => banner.id !== id)
    );
  };
  ...
  const handleDeleteConfirm = (
    ...
    if (bannerToDelete) {
      setFieldValue(
        "bannerImages",
        bannerImages.filter((banner) => banner.id !== bannerToDelete.id)
      );
    }
    ...
    try {
      ...
      if (result.status === 1) {
        ...
        if (responseData?.savedMedia?.id) {
          ...
          console.log(
            "Small banner file upload - savedMedia.id:",
            responseData.savedMedia.id
          );
        }
      } else {
  ...
  const onSubmit = async (values: HeroSectionFormSchema) => {
    try {
      ...
      const bannerBlock = heroBlocks.find(
        (block: ContentBlock) =>
          block.media_files &&
          block.media_files.length > 0 &&
          (block.title?.toLowerCase() === "banner image" ||
      ...
      const smallBannerBlock = heroBlocks.find(
        (block: ContentBlock) => block.title === "Small Banner"
      );
      ...
      const headerTextChanged =
        headlineBlock && values.headerText !== initialValues.headerText;
      const headerHeadingLevelChanged =
        headlineBlock &&
        values.headerHeadingLevel !== initialValues.headerHeadingLevel;
        ...
        if (headerHeadingLevelChanged) {
          ...
          if (customCss.match(/heading-level:\s*h[1-6]/i)) {
            customCss = customCss.replace(
              ...
              `heading-level:${values.headerHeadingLevel}`
            );
          } else {
            customCss = customCss
              ? `${customCss}; heading-level:${values.headerHeadingLevel}`
              : `heading-level:${values.headerHeadingLevel}`;
          }
        }
      ...
      const subHeaderTextChanged =
        subtitleBlock && values.subHeaderText !== initialValues.subHeaderText;
      const subHeaderHeadingLevelChanged =
        subtitleBlock &&
        values.subHeaderHeadingLevel !== initialValues.subHeaderHeadingLevel;
      ...
      if (subHeaderTextChanged || subHeaderHeadingLevelChanged) {
        ...
        if (subHeaderHeadingLevelChanged) {
          ...
          if (customCss.match(/heading-level:\s*h[1-6]/i)) {
            customCss = customCss.replace(
              /heading-level:\s*h[1-6]/i,
              `heading-level:${values.subHeaderHeadingLevel}`
            );
          } else {
            customCss = customCss
              ? `${customCss}; heading-level:${values.subHeaderHeadingLevel}`
              : `heading-level:${values.subHeaderHeadingLevel}`;
          }
        }
        updatedContentBlocks.push({
      ...
      const smallBannerFileChanged =
        values.smallBannerFile !== initialValues.smallBannerFile;
      const smallBannerMediaFileIdChanged =
        values.smallBannerMediaFileId !== initialValues.smallBannerMediaFileId;
      const smallBannerChanged =
        smallBannerBlock &&
        (smallBannerFileChanged || smallBannerMediaFileIdChanged);

      console.log("Small Banner comparison:", {
        file: {
          current: values.smallBannerFile,
          initial: initialValues.smallBannerFile,
          changed: smallBannerFileChanged,
        },
        mediaFileId: {
          current: values.smallBannerMediaFileId,
          initial: initialValues.smallBannerMediaFileId,
          changed: smallBannerMediaFileIdChanged,
        },
                ...
                <FormContainer>
                            ...
                            {({ field, form }: any) => (
                              <Select
                                ...
                                value={headingLevelOptions?.find(
                                  (option) =>
                                    option.value === values?.headerHeadingLevel
                                )}
                              />
                            )}
                          </Field>
                  ...
                  <div className="mb-6 rounded-[24px] border-[0.75px] border-[#CED4DA] p-4">
                    <div className="flex flex-col sm:flex-row  sm:justify-between !items-baseline mb-4 border-b border-[#CED4DA]">
                      <h3 className="text-[#495057] font-inter text-[14px] font-semibold leading-normal mb-[20px]">
                        Banner Images
                      </h3>
                      <Button
                        type="button"
                        ...
                        <div className="flex flex-col sm:flex-row gap-3">
                          <label className="cursor-pointer">
                            ...
                            <Button
                              ...
                              loading={isUploading}
                              onClick={() =>
                                smallBannerFileRef.current?.click()
                              }
                ...
                <Modal
                  ...
                  style={{
                    ...
                    overlay: {
                      ...
                      top: 0,
                      left:
                        window.innerWidth >= 1024 ? `${SIDE_NAV_WIDTH}px` : "0",
                      right: 0,
                      bottom: 0,
                ...
                >
                  <div className="p-3 sm:p-4 md:p-6">
                    ...
                    >
                      {({
                        values: modalValues,
                        setFieldValue: modalSetFieldValue,
                        touched,
                        errors,
                        isSubmitting,
                      }) => (
                        <Form>
                          <FormContainer>
                            <div className="space-y-4 sm:space-y-6">
                              <FormItem
                                label={
                                  editingBanner && editingBanner.id === "1"
                                    ? "Banner Video Name"
                                    : "Banner Image Name"
                                }
                                labelClass="text-[#495057] font-inter text-[12px] sm:text-[14px] font-medium leading-normal"
                                invalid={
                                  (errors.bannerImageName &&
                                    touched.bannerImageName) as boolean
                                }
                              ...
                              >
                                <Field
                                  ...
                                  placeholder={
                                    editingBanner && editingBanner.id === "1"
                                      ? "Enter banner video name"
                                      : "Enter banner image name"
                                  }
                                />
                              </FormItem>
                                ...
                                <div className="w-full">
                                  <div className="flex flex-col w-full rounded-[24px] border-[0.75px] border-[#CED4DA] p-2 sm:p-3">
                                    <div className="flex-1 mb-2">
                                      <span className="text-gray-700 font-medium text-[12px] sm:text-[14px] break-all">
                                        {modalValues.imageFile}
                                      </span>
                                    </div>
                                    <div className="flex justify-end">
                                      <label className="cursor-pointer">
                  ...
                  <div className="p-6 sm:p-8">
                    <div className="text-center">
                      ...
                      <p className="text-[#6B7280] font-inter text-[14px] sm:text-[16px] leading-normal mb-6">
                        Could you please confirm if you want to remove this
                        Banner Image?
                      </p>
